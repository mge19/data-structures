...
bool Tree::searchRecords(treenode *current, char *targetNamePart) {
	if (strncmp(targetNamePart, current->name,1) == 0) {
		cout << "Found: " << current->name << endl;
		current->phonenum.printNumList();
		return true;
	}
	else if (current->right && strncmp(targetNamePart, current->name,1) > 0)
		return searchRecords(current->right, targetNamePart);
	else if (current->left && memcmp(targetNamePart, current->name,1) < 0)
